    ...
    <ul class="links-desktop nav nav-pills col text-center align-items-center justify-content-center ">
      <li v-for="link in this.links">
        <router-link v-if="(link.loginNeeded && this.isLogged) || (!link.loginNeeded)"
                     :to="link.href" class="nav-item nav-link px-2 navbar-links">{{ link.name }}
        </router-link>
      </li>

      <li>
        <div v-if="this.isLogged" class="position-relative">
          <router-link :to="this.linkToNotification" class="nav-item nav-link px-2 navbar-links">{{
              "Notifications"
            }}
          </router-link>
    ...
    <div class="collapse menu-mobile" id="menuMobileCollapse" ref="collapse">
      <div class="collapse-body">
        <ul>
          ...
          <li>
            <div v-if="this.isLogged" class="position-relative">
              <router-link :to="this.linkToNotification" class="navbar-links" @click="this.collapse">{{
                  "Notifications"
                }}
              </router-link>
              <hr/>
...
export default {
  ...
  data() {
    return {
      ...
      links: [{href: "/", name: "Home", loginNeeded: false},
        {href: "/addwhiteboard", name: "Whiteboards", loginNeeded: true}, {
          href: "/profile",
          name: "Profile",
          loginNeeded: true
        },],
  ...
  emits: ['onBadToken', 'joinApplication'],
  created() {
    ...
    reloadNavbar() {
      if (localStorage.getItem('accessToken') && !this.isLogged) {
        axios.post('http://localhost:4000/auth/refresh', {
          ...
          socket.emit("joinApplication", localStorage.getItem('accessToken'), (response) => {
            if (response.status === 'ok') {
              this.$emit('joinApplication');
            } else {
              console.error('Errore nella join application')
            }
          });
          this.isLogged = true;
